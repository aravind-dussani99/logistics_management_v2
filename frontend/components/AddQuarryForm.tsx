...
import { QuarryOwner, RateEntry } from '../types';
import { safeToFixed } from '../utils';

interface AddQuarryFormProps {
    onClose: () => void;
}
...
const InputField: React.FC<React.InputHTMLAttributes<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement> & {label: string}> = ({ label, ...props }) => {
    ...
    return (
        <div className="col-span-1">
            ...
            {props.type === 'select' ? (
                 <select {...props as React.SelectHTMLAttributes<HTMLSelectElement>} id={inputId} name={inputName} className="mt-1 block w-full px-3 py-2 rounded-md bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-600 focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary sm:text-sm" />
            ) : props.type === 'textarea' ? (
...
const AddQuarryForm: React.FC<AddQuarryFormProps> = ({ onClose }) => {
    const { addQuarryOwner, materials, siteLocations } = useData();
    const pickupSites = siteLocations.filter(site => site.type === 'pickup' || site.type === 'both');
    const [ownerData, setOwnerData] = useState<Omit<QuarryOwner, 'id' | 'rates'>>({
        ownerName: '', quarryName: '', contactNumber: '', quarryArea: 0, address: '', openingBalance: 0
    });
    ...
    useEffect(() => {
        ...
        let gstAmt = 0;
        if (gstPercent > 0) {
            if (rateData.gst === 'inclusive') gstAmt = baseRate - (baseRate / (1 + (gstPercent / 100)));
            else { gstAmt = baseRate * (gstPercent / 100); total = baseRate + gstAmt; }
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        const newQuarry: Omit<QuarryOwner, 'id'> = { ...ownerData, rates: [{ ...rateData, id: uuidv4() }] };
        await addQuarryOwner(newQuarry);
        onClose();
    };
    ...
    return (
        <form onSubmit={handleSubmit} className="p-8">
            <div className="space-y-10">
                <div>
                    <h3 className="text-xl font-semibold leading-6 text-gray-900 dark:text-white">1. Quarry Owner Details</h3>
                    <div className="mt-6 grid grid-cols-1 gap-y-6 gap-x-6 sm:grid-cols-2">
                       ...
                       <InputField label="Quarry Name" id="quarryName" name="quarryName" type="text" value={ownerData.quarryName} onChange={handleOwnerChange} required />
                       <InputField label="Contact Number" id="contactNumber" name="contactNumber" type="text" value={ownerData.contactNumber} onChange={handleOwnerChange} />
