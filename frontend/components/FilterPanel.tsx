...
const FilterPanel: React.FC<FilterPanelProps> = ({ filters, setFilters, data, visibleFields = defaultVisibleFields }) => {
    ...
    const handleFilterChange = (key: keyof Filters, value: string) => {
        ...
        if (hasSingleDate && key === 'dateFrom') {
            setFilters(prev => ({ ...prev, dateFrom: value, dateTo: value }));
            return;
        }
    ...
    const resetFilters = () => {
        ...
        setFilters(hasSingleDate ? { dateFrom: nextDate, dateTo: nextDate } : { dateFrom: formatDate(startOfMonth), dateTo: nextDate });
    };
