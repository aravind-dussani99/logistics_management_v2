...
const MineQuarryDataPage: React.FC = () => {
  ...
  const filteredRows = useMemo(() => {
    ...
    if (!term) return mineQuarries;
    return mineQuarries.filter(row => {
  ...
  }, [mineQuarries, merchantTypes, siteLocations, searchTerm]);
  ...
  const handleAdd = () => {
    openModal('Add Mine & Quarry', <ProfileForm onSave={handleCreate} onClose={closeModal} />);
  };

  const handleCreate = async (data: Omit<MineQuarryData, 'id' | 'merchantTypeName' | 'siteLocationName'>) => {
    await addMineQuarry(data);
    setCurrentPage(1);
    closeModal();
  };

  const handleEdit = (row: MineQuarryData) => {
    openModal('Edit Mine & Quarry', <ProfileForm initialData={row} onSave={(data) => handleUpdate(row.id, data)} onClose={closeModal} />);
  };

  const handleUpdate = async (id: string, data: Omit<MineQuarryData, 'id' | 'merchantTypeName' | 'siteLocationName'>) => {
    await updateMineQuarry(id, data);
    closeModal();
  };
  ...
  const handleBulkDelete = async () => {
    ...
    const shouldDelete = await confirm('Delete Selected', `Delete ${selectedIds.size} mine/quarry record(s)?`);
    if (!shouldDelete) return;
      ...
      <main className="pt-6 space-y-4">
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-4 flex flex-wrap gap-4 items-end">
          <div className="min-w-[220px] flex-1">
            ...
            <input
              ...
              placeholder="Mine & quarry, type, site"
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden">
          <div className="p-4 border-b dark:border-gray-700 flex justify-between items-center">
            <h2 className="text-xl font-semibold">Mine & Quarry List</h2>
            ...
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              <thead className="bg-gray-50 dark:bg-gray-700">
                <tr>
                  ...
                  {['S. No.', 'Merchant Type', 'Mine & Quarry Name', 'Contact', 'Email', 'Site Location', 'Company', 'GST Opt-in', 'GST Number', 'GST Details', 'Remarks', 'Actions'].map(header => (
  ...
  function ProfileForm({ initialData, onSave, onClose }: {
    initialData?: MineQuarryData;
    onSave: (data: Omit<MineQuarryData, 'id' | 'merchantTypeName' | 'siteLocationName'>) => void;
    onClose: () => void;
  }) {
    const [formData, setFormData] = useState<Omit<MineQuarryData, 'id' | 'merchantTypeName' | 'siteLocationName'>>({
    ...
    return (
      <form onSubmit={handleSubmit} className="p-8 space-y-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <InputField label="Mine & Quarry Name" id="name" name="name" value={formData.name} onChange={handleChange} required />
...
export default MineQuarryDataPage;
