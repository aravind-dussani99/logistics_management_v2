...
const PaymentImport: React.FC = () => {
  ...
  }, [rows.length, importMode, headerOverrides]);
  ...
  const validateAndParse = () => {
    ...
    for (let index = 1; index < rows.length; index += 1) {
      ...
      const transactionType = getValueFromRow(row, HEADER_ALIASES.transactionType, 'transactionType');
      const fromAccount = getValueFromRow(row, HEADER_ALIASES.fromAccount, 'fromAccount');
      const toName = getValueFromRow(row, HEADER_ALIASES.toName, 'toName');
      const amountValue = normalizeAmount(getValueFromRow(row, HEADER_ALIASES.amount, 'amount'));
      const remarks = getValueFromRow(row, HEADER_ALIASES.remarks, 'remarks');
      parsed.push({
        rowNumber,
        data: {
          date: dateValue,
          ...
          amount: amountValue,
          remarks,
          toAccount: getValueFromRow(row, HEADER_ALIASES.toAccount, 'toAccount'),
          via: getValueFromRow(row, HEADER_ALIASES.via, 'via'),
          headAccount: getValueFromRow(row, HEADER_ALIASES.headAccount, 'headAccount'),
          category: getValueFromRow(row, HEADER_ALIASES.category, 'category'),
          subCategory: getValueFromRow(row, HEADER_ALIASES.subCategory, 'subCategory'),
          tripId: getValueFromRow(row, HEADER_ALIASES.tripId, 'tripId'),
        },
      });
    }
