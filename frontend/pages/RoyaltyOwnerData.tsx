...
const RoyaltyOwnerDataPage: React.FC = () => {
  ...
  const filteredRows = useMemo(() => {
    ...
    if (!term) return royaltyOwnerProfiles;
    return royaltyOwnerProfiles.filter(row => {
  ...
  }, [royaltyOwnerProfiles, merchantTypes, siteLocations, searchTerm]);
  ...
  const handleCreate = async (data: Omit<RoyaltyOwnerData, 'id' | 'merchantTypeName' | 'siteLocationName'>) => {
    await addRoyaltyOwnerProfile(data);
    setCurrentPage(1);
    closeModal();
  };

  const handleEdit = (row: RoyaltyOwnerData) => {
    openModal('Edit Royalty Owner', <ProfileForm initialData={row} onSave={(data) => handleUpdate(row.id, data)} onClose={closeModal} />);
  };

  const handleUpdate = async (id: string, data: Omit<RoyaltyOwnerData, 'id' | 'merchantTypeName' | 'siteLocationName'>) => {
    await updateRoyaltyOwnerProfile(id, data);
    closeModal();
  };
  ...
  const handleMerge = (row: RoyaltyOwnerData) => {
    const options = royaltyOwnerProfiles.filter(item => item.id !== row.id).map(item => ({ id: item.id, name: item.name }));
    if (options.length === 0) {
      openModal('Merge Royalty Owner', <AlertDialog message="No other record available to merge into." onConfirm={closeModal} />);
      return;
    }
    openModal('Merge Royalty Owner', (
      <MergeDialog
        sourceLabel="Royalty Owner"
        sourceName={row.name}
        options={options}
        onConfirm={async (targetId) => {
          await mergeRoyaltyOwnerProfile(row.id, targetId);
          closeModal();
        }}
        onClose={closeModal}
      />
  ...
  return (
    <div className="relative">
      <PageHeader
        title="Royalty Owner Data"
        subtitle="Manage royalty owner profiles with GST details."
        filters={{}}
        onFilterChange={() => {}}
        filterData={{ vehicles: [], transportOwners: [], customers: [], quarries: [], royaltyOwners: [] }}
        pageAction={{ label: 'Add Royalty Owner', action: handleAdd }}
      />
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-4 flex flex-wrap gap-4 items-end">
          <div className="min-w-[220px] flex-1">
            ...
            <input
              ...
              placeholder="Royalty owner, type, site"
  ...
  function ProfileForm({ initialData, onSave, onClose }: {
    initialData?: RoyaltyOwnerData;
    onSave: (data: Omit<RoyaltyOwnerData, 'id' | 'merchantTypeName' | 'siteLocationName'>) => void;
    onClose: () => void;
  }) {
    const [formData, setFormData] = useState<Omit<RoyaltyOwnerData, 'id' | 'merchantTypeName' | 'siteLocationName'>>({
    ...
    return (
      <form onSubmit={handleSubmit} className="p-8 space-y-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <InputField label="Royalty Owner Name" id="name" name="name" value={formData.name} onChange={handleChange} required />
...
export default RoyaltyOwnerDataPage;
