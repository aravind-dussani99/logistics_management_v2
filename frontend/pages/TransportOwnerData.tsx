...
const TransportOwnerDataPage: React.FC = () => {
  const { transportOwnerProfiles, merchantTypes, siteLocations, addTransportOwnerProfile, updateTransportOwnerProfile, deleteTransportOwnerProfile, mergeTransportOwnerProfile, loadTransportOwnerProfiles, loadMerchantTypes, loadSiteLocations, refreshKey } = useData();
  ...
  const filteredRows = useMemo(() => {
    ...
    if (!term) return transportOwnerProfiles;
    return transportOwnerProfiles.filter(row => {
  ...
  }, [transportOwnerProfiles, merchantTypes, siteLocations, searchTerm]);
  ...
  const handleAdd = () => {
    openModal('Add Transport & Owner', <ProfileForm onSave={handleCreate} onClose={closeModal} />);
  };

  const handleCreate = async (data: Omit<TransportOwnerData, 'id' | 'merchantTypeName' | 'siteLocationName'>) => {
    await addTransportOwnerProfile(data);
    setCurrentPage(1);
    closeModal();
  };

  const handleEdit = (row: TransportOwnerData) => {
    openModal('Edit Transport & Owner', <ProfileForm initialData={row} onSave={(data) => handleUpdate(row.id, data)} onClose={closeModal} />);
  };

  const handleUpdate = async (id: string, data: Omit<TransportOwnerData, 'id' | 'merchantTypeName' | 'siteLocationName'>) => {
    await updateTransportOwnerProfile(id, data);
    closeModal();
  };
  ...
  const handleMerge = (row: TransportOwnerData) => {
    const options = transportOwnerProfiles.filter(item => item.id !== row.id).map(item => ({ id: item.id, name: item.name }));
    if (options.length === 0) {
      openModal('Merge Transport Owner', <AlertDialog message="No other record available to merge into." onConfirm={closeModal} />);
      return;
    }
    openModal('Merge Transport Owner', (
      <MergeDialog
        sourceLabel="Transport Owner"
        sourceName={row.name}
        options={options}
        onConfirm={async (targetId) => {
          await mergeTransportOwnerProfile(row.id, targetId);
          closeModal();
        }}
        onClose={closeModal}
      />
  ...
  const handleBulkDelete = async () => {
    ...
    const shouldDelete = await confirm('Delete Selected', `Delete ${selectedIds.size} transport owner record(s)?`);
    if (!shouldDelete) return;
      ...
      <main className="pt-6 space-y-4">
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-4 flex flex-wrap gap-4 items-end">
          <div className="min-w-[220px] flex-1">
            ...
            <input
              ...
              placeholder="Transport owner, type, site"
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden">
          <div className="p-4 border-b dark:border-gray-700 flex justify-between items-center">
            <h2 className="text-xl font-semibold">Transport & Owner List</h2>
            ...
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              <thead className="bg-gray-50 dark:bg-gray-700">
                <tr>
                  ...
                  {['S. No.', 'Merchant Type', 'Transport & Owner Name', 'Contact', 'Email', 'Site Location', 'Company', 'GST Opt-in', 'GST Number', 'GST Details', 'Remarks', 'Actions'].map(header => (
                ...
                {paginatedRows.length === 0 && (
                  <tr>
                    <td colSpan={13} className="px-6 py-10 text-center text-sm text-gray-500 dark:text-gray-400">
                      No transport owner records yet.
                    </td>
                  </tr>
                )}
              </tbody>
  ...
  function ProfileForm({ initialData, onSave, onClose }: {
    initialData?: TransportOwnerData;
    onSave: (data: Omit<TransportOwnerData, 'id' | 'merchantTypeName' | 'siteLocationName'>) => void;
    onClose: () => void;
  }) {
    const [formData, setFormData] = useState<Omit<TransportOwnerData, 'id' | 'merchantTypeName' | 'siteLocationName'>>({
    ...
    return (
      <form onSubmit={handleSubmit} className="p-8 space-y-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <InputField label="Transport & Owner Name" id="name" name="name" value={formData.name} onChange={handleChange} required />
...
export default TransportOwnerDataPage;
