...
import { QuarryOwner as QuarryOwnerType, RateEntry } from '../types';
import { useData } from '../contexts/DataContext';
import { useUI } from '../contexts/UIContext';
import PageHeader from '../components/PageHeader';
import { Filters } from '../components/FilterPanel';
import AddQuarryForm from '../components/AddQuarryForm';
...
const QuarriesPage: React.FC = () => {
    ...
    const flatData = useMemo(() => quarries.map(q => ({
        ...q,
        activeRate: getActiveRate(q.rates)
    })), [quarries]);
    ...
    const handleSave = async (quarryId: string, data: Partial<RateEntry>) => {
        if ('id' in data) {
            await updateQuarryRate(quarryId, data as RateEntry);
        } else {
            await addQuarryRate(quarryId, data as Omit<RateEntry, 'id'>);
        }
        closeModal();
    };

    const handleEdit = (quarry: QuarryOwnerType, rate: RateEntry) => {
        openModal(`Edit Rate for ${quarry.quarryName}`, <QuarryForm quarry={quarry} initialData={rate} onSave={handleSave} onClose={closeModal} />);
    };
    
    const handleAddNewRate = (quarry: QuarryOwnerType) => {
         openModal(`Add New Rate for ${quarry.quarryName}`, <QuarryForm quarry={quarry} onSave={handleSave} onClose={closeModal} />);
    }
    
    const handleAddQuarry = () => {
        openModal('Add New Quarry', <AddQuarryForm onClose={closeModal} />);
    }

    const handleDelete = async (quarryId: string, rateId: string) => {
        if (window.confirm('Are you sure you want to delete this rate?')) {
            await deleteQuarryRate(quarryId, rateId);
        }
    };
    
    const popoverData = useMemo(() => {
        if (!activePopover) return null;
        return quarries.find(q => q.id === activePopover);
    }, [activePopover, quarries]);
            ...
            <main className="pt-6">
                <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden">
                    <div className="p-4 border-b dark:border-gray-700 flex justify-between items-center">
                        <h2 className="text-xl font-semibold">Quarry Rates List</h2>
                        <p className="text-sm text-gray-500">Showing current active rate for each quarry</p>
                    </div>
                    <div className="overflow-x-auto">
                        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                             <thead className="bg-gray-50 dark:bg-gray-700">
                                <tr>
                                    {['Owner', 'From Site', 'Material', 'Rate/Ton', 'Total Rate', 'Effective From', 'To', 'Status', 'Actions'].map(h => <th key={h} className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">{h}</th>)}
                                </tr>
                            </thead>
                            <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                {flatData.map(q => (
                                    <tr key={q.id}>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">{q.ownerName}</td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm">{q.activeRate?.fromSite || '-'}</td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm">{q.activeRate?.materialType || '-'}</td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm">{formatCurrency(q.activeRate?.ratePerTon)}</td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm font-bold">{formatCurrency(q.activeRate?.totalRate)}</td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm">{q.activeRate?.effectiveFrom || '-'}</td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm">{q.activeRate?.effectiveTo || 'Present'}</td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm">
                                            {q.activeRate?.active === 'active' 
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                                            <button onClick={() => handleAddNewRate(q)} className="px-3 py-2 text-sm font-medium text-white bg-green-600 rounded-md hover:bg-green-700">New Rate</button>
                                            <button onClick={() => q.activeRate && handleEdit(q, q.activeRate)} disabled={!q.activeRate} className="px-3 py-2 text-sm font-medium text-white bg-blue-600 rounded-md hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed">Edit</button>
                                            <button onClick={() => q.activeRate && handleDelete(q.id, q.activeRate.id)} disabled={!q.activeRate} className="px-3 py-2 text-sm font-medium text-white bg-red-600 rounded-md hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed">Delete</button>
                                            <button onClick={() => setActivePopover(activePopover === q.id ? null : q.id)} className="px-3 py-2 text-sm font-medium text-gray-700 bg-gray-200 rounded-md hover:bg-gray-300 dark:bg-gray-600 dark:text-gray-200 dark:hover:bg-gray-500">Old Rates</button>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
...
const QuarryForm: React.FC<{ quarry: QuarryOwnerType, initialData?: Partial<RateEntry>, onSave: (quarryId: string, data: Partial<RateEntry>) => void, onClose: () => void }> = ({ quarry, initialData, onSave, onClose }) => {
    ...
    useEffect(() => {
        ...
        if (gstPercent > 0) {
            if (formData.gst === 'inclusive') {
                 gstAmt = baseRate - (baseRate / (1 + (gstPercent / 100)));
            } else { // exclusive
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                ...
                <div className="sm:col-span-2 md:col-span-3 flex items-center space-x-2">
                    <input type="checkbox" name="active" id="active-rate-quarry" checked={formData.active === 'active'} onChange={handleChange} className="h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary" />
                    <label htmlFor="active-rate-quarry">Set as Active Rate</label>
                </div>
            </div>
